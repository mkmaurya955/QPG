...
if (isset($_POST['login'])){
		...
		if (count($errors)==0) {
			...
			$sql1="SELECT * FROM admin WHERE Admin_ID='$username' AND Password='$password'";
			...
			if (mysqli_num_rows($res)==1) {
				...
				header('location:admin/admindash.php'); // redirect to the home page
			}else{
		...
		if (count($errors)==0) {
			$password=$password;
			//$password=md5($password); // encrpt password before comparing with that from database
			$sql1= "SELECT * FROM Teacher WHERE '$username'=Teacher_ID AND '$password'=Password";
			$res=mysqli_query($con, $sql1);
